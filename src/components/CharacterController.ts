...
// @ts-ignore
  ...
  private _doMove(dt: number): ActionData {
    ...
    if (this._inFreeFall) {
      ...
      switch (true) {
        case (this._act._stepLeft && !this._act._walk && !this._act._walkback):
          ...
          moving = true;
          break;
        case (this._act._stepRight && !this._act._walk && !this._act._walkback):
          sign = -this._signRHS * this._isAvFacingCamera();
  ...
  private _snapCamera() {
    ...
    const pi: PickingInfo = this._scene.pickWithRay(this._ray, (mesh) => {
      ...
      return !(mesh == this._avatar || !mesh.checkCollisions);
    }, true);
  ...
  public setAvatarSkeleton(skeleton: Skeleton) {
    ...
    // @ts-ignore
    this._isAG = !!(this._skeleton != null && this._skeleton.overrideMesh);
...
export class ActionMap {
  ...
  public turnLeft = new ActionData("turnLeft", Math.PI / 8, "q");
  public turnLeftFast = new ActionData("turnLeftFast", Math.PI / 4, "na");
  public turnRight = new ActionData("turnRight", Math.PI / 8, "e");
  public turnRightFast = new ActionData("turnRightFast", Math.PI / 4, "na");
  public strafeLeft = new ActionData("strafeLeft", 1.5, "a");
